...
contract GokuTokenWrapper {
    using SafeMath for uint256;
    IERC20 public goku;

    constructor(IERC20 _gokuAddress) public {
        goku = IERC20(_gokuAddress);
    }
    ...
    function stake(uint256 amount) public {
        ...
        goku.transferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount) public {
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        goku.transfer(msg.sender, amount);
    }
}

contract GenesisPool is GokuTokenWrapper, Ownable {
    ERC1155Tradable public gokus;
    ...
    constructor(ERC1155Tradable _gokusAddress, IERC20 _gokuAddress) public GokuTokenWrapper(_gokuAddress) {
        gokus = _gokusAddress;
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        return
        points[account].add(
            blockTime.sub(lastUpdateTime[account]).mul(balanceOf(account)).div(86400)
        );
    }

    // stake visibility is public as overriding GokuTokenWrapper's stake() function
    function stake(uint256 amount) public updateReward(msg.sender) {
        require(amount.add(balanceOf(msg.sender)) <= 10000 ether, "Cannot stake more than 10000 GOKU");
    ...
    function redeem(uint256 card) public updateReward(msg.sender) {
        ...
        require(gokus.totalSupply(card) < gokus.maxSupply(card), "Max cards minted");

        points[msg.sender] = points[msg.sender].sub(cards[card]);
        gokus.mint(msg.sender, card, 1, "");
